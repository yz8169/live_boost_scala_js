...
class AdminController @Inject()(cc: ControllerComponents, userDao: UserDao, formTool: FormTool,
        ...
        val f = Future {
          val content =
            s"""
               ...
               |绘云生物科技
         """.stripMargin
          val info = Info("LiveBoost用户权限变动通知", content)
          val inbox = x.email
  ...
  def getAllMission = Action.async { implicit request =>
    missionDao.selectAll.zip(extraDataDao.selectAll).zip(userDao.selectAll).
      ...
      val array = Utils.getArrayByT2s(t2s).map { x =>
        x + ("userName" -> userMap.getOrElse(x("userId").toInt, "已删")) + ("isDelete" -> trashMap(x("id").toInt))
      }
      Ok(Json.toJson(array))
    }
  }
  ...
  def downloadDb = Action.async { implicit request =>
    missionDao.selectAll.zip(extraDataDao.selectAll).zip(userDao.selectAll).
      ...
      val headers = ArrayBuffer("用户", "样品号", "姓名", "年龄", "AST", "ALT", "PLT", "风险得分", "结果", "时间", "是否删除",
      ...
      lines += headers ++= missionRows.map { x =>
        ...
        ArrayBuffer(userName, extra.sampleId, extra.name, x.age, x.ast, x.alt, x.plt, x.score, x.result, Utils.dataTime2String(x.endTime), isDelete,
